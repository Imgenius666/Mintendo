...
public class TetrisBoard implements Serializable{
    ...
    public int placementHeight(TetrisPiece piece, int x) {
        int[] ly = piece.getLowestYVals();
        int res = 0;
        for (int i = 0; i < ly.length; i++) {
            int h1 = getColumnHeight(x + i);
            if ((x + i) >= width){
                break;
            }
            int res1 = h1 - ly[i];
    ...
    public int placePiece(TetrisPiece piece, int x, int y){
        if (!committed){
            throw new RuntimeException("the commit is false");
        }
        int res = 0;
        committed = false;
        this.backupGrid();
        for (TetrisPoint point : piece.getBody()) {
            ...
            if (x_1 < 0 || x_1 >= width || y_1 < 0 || y_1 >= height) {
                res = TetrisBoard.ADD_OUT_BOUNDS;
                undo();
                break;
            } else if (tetrisGrid[x_1][y_1]) {
                res = TetrisBoard.ADD_BAD;
                undo();
                break;
            } else {
                tetrisGrid[x_1][y_1] = true;
                makeHeightAndWidthArrays();
                if (width == rowCounts[y_1]) {
                    res = TetrisBoard.ADD_ROW_FILLED;
                } else {
                    res = TetrisBoard.ADD_OK;
                }
            }
        }
        return res;
    ...
    public int clearRows() {
        ...
        for (int q = 0; q < tetrisGrid.length; q++) {
            System.arraycopy(tetrisGrid[q], 0, grid[q], 0, tetrisGrid[q].length);
        }
        int res = 0;
        ...
        for (int i = 0; i <= maxh; i++) {
            if (rowCounts[i] == width) {
                for (int j = 0; j < width; j++) {
                    tetrisGrid[j][i - res] = grid[j][i + 1];
                }
                res += 1;
            } else {
                for (int j = 0; j < width; j++) {
                    tetrisGrid[j][i - res] = grid[j][i];
                }
            }
        }
        makeHeightAndWidthArrays();
