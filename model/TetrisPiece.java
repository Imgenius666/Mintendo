    ...
    */
    ...
    public TetrisPiece(TetrisPoint[] points) {
        ...
        for(TetrisPoint p: this.body){
            if (!(a.containsKey(p.x))) {
                a.put(p.x, p.y);
            }if (h < p.y){
                h = p.y;
            }else if(a.get(p.x) > p.y) {
                a.put(p.x, p.y);
            }
        }
        int i = 0;
        this.lowestYVals = new int[a.size()];
        for (Integer p: a.keySet()){
            this.lowestYVals[i] = a.get(p);
            i ++;
        }
    ...
    public boolean equals(Object obj) {
        if (this == obj){
            ...
            TetrisPiece p = (TetrisPiece) obj;
            return (this.body.length == p.body.length) && (Arrays.asList(this.body).containsAll(Arrays.asList((p.body))));
        }
        }
    ...
    public static TetrisPiece makeFastRotations(TetrisPiece root) {
        TetrisPiece c = root;
        while(true) {
            TetrisPiece cc = c.computeNextRotation();
            if (root.equals(cc)) {
                c.next = root;
                break;
            } else {
    ...
    public TetrisPiece computeNextRotation() {
        int i = 0;
        TetrisPoint[] rr = new TetrisPoint[this.body.length];
        for(TetrisPoint p: this.body){
            int x1 = p.y;
            int y1 = this.width - 1  - p.x;
            TetrisPoint a = new TetrisPoint(x1, y1);
            rr[i] = a;
            i++;
            }
        TetrisPiece res = new TetrisPiece(rr);
        return res;
    }
    /**
